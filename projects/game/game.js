...
function updateGameStage() {
    if (currentStage === 1) {
        ...
        luffyMaxHP += 20; 
        luffyHP = luffyMaxHP;
        storyBox.innerHTML = "You defeated Alvida! You sailed to Shells Town and saved the pirate hunter, Roronoa Zoro. He joins your crew! <strong>(Max HP increased by 20!)</strong>";
        updateUI();
    }
        ...
        storyBox.innerHTML = "You arrive at Orange Town. The flashy pirate Buggy the Clown is terrorizing the town! Time to fight!";
        updateUI();
    }
        ...
        luffyMaxHP += 20;
        luffyHP = luffyMaxHP;
        storyBox.innerHTML = "Buggy is blasted away! You travel to Syrup Village, get a ship (The Going Merry), and the sniper Usopp joins the crew!";
        updateUI();
    }
        ...
        storyBox.innerHTML = "At the sea restaurant Baratie, Sanji the cook joins you! But suddenly, the pirate Don Krieg attacks the restaurant!";
        updateUI();
        }
        ...
        storyBox.innerHTML = "Krieg is defeated! You chase your navigator Nami to Arlong Park. To save her village, you must defeat the Fishman Arlong!";
        updateUI();
    }
        ...
        luffyHP = luffyMaxHP;
        storyBox.innerHTML = "Arlong's park is destroyed! Nami is free! You finally enter the Grand Line and arrive at the desert kingdom of Alabasta.";
        updateUI();
     }
     else if (currentStage === 8) {
        isCombat = true;
        currentLocation = "Alabasta Tombs";
        currentBoss = { name: "Crocodile", hp: 150, maxHp: 150, color: "darkgreen" };
        storyBox.innerHTML = "The Warlord Crocodile is trying to take over Alabasta! He is made of sand, but you brought water! Fight for the kingdom!";
        luffyMaxHP += 30;
        luffyHP = luffyMaxHP;
        updateUI();
    }
        ...
        storyBox.innerHTML = "CROCODILE IS DEFEATED! But the Marines are surrounding the island! Smoker and Hina are closing in! What do you do?";
...
function finishGame(choice) {
    ...
    storyControls.innerHTML = ""; 
    if (choice === 'fight') {
        storyBox.innerHTML = "You tried to fight an entire Marine fleet... You fought bravely, but got captured and sent to Impel Down. <strong>GAME OVER.</strong> <button onclick='location.reload()'>Restart</button>";
    } else {
        storyBox.innerHTML = "You ran past the Marines, said a tearful goodbye to Princess Vivi, and set sail on the Going Merry! Your bounty has skyrocketed! <strong>YOU WIN! THE ROMANCE DAWN CONTINUES!</strong> <button onclick='location.reload()'>Play Again</button>";
    }
}
...
function playerAction(actionType) {
    ...
    if (luffyHP <= 0) {
        ...
        updateUI(); 
        storyControls.style.display = "none"; 
        storyBox.innerHTML = storyLog + "<br><br><strong style='color:red;'>GAME OVER.</strong><br>You were knocked out! <button onclick='location.reload()'>Restart Game</button>";
    } else if (currentBoss.hp <= 0) {
        isCombat = false; 
        storyBox.innerHTML = storyLog + `<br><br><strong>${currentBoss.name} Defeated!</strong> Click Continue to sail on.`;
        updateUI(); 
    } else {
